...
jqSimpleConnect._positionConnection = function(connection) {
	...
	var layout = connection.target.offset();
	posA.top =  posA.top - layout.top;
	posB.top =  posB.top - layout.top;
	posA.left =  posA.left - layout.left;
	posB.left =  posB.left - layout.left;
...
jqSimpleConnect.connect = function(elementA, elementB, options) {
	...
	connection.id = (options != null && options.id != null) ? options.id : 'jqSimpleConnect_'+jqSimpleConnect._idGenerator++;
	connection.elementA = elementA;
	connection.elementB = elementB;
	connection.zindex = (options != null && options.zindex != null)? options.zindex : '0';
	connection.color = (options != null && options.color != null)? options.color + '' : '#808080';
	connection.highlight = (options != null && options.highlight != null)? options.highlight + '' : '#55AAFF';
	...
	target.prepend(div.replace('divUniqueIdentifier', connection.id + '_1'))
		.prepend(div.replace('divUniqueIdentifier', connection.id + '_2'))
		.prepend(div.replace('divUniqueIdentifier', connection.id + '_3'))
	;

	// add by github.com/arielbottero ------------------------------------------
